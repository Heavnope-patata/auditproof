...
**Let AI trading agents prove they followed the rules -- without anyone handing over their data.**

NTU-CCTF x SNZ InnovateX 2026 / Track 2
...
```bash
npm run witness:custodian   # three witnesses in three terminals
...
```
auditproof/
+- circuits/              audit.circom (core circuit) + build.sh (one-shot compile and setup)
+- policy-compiler/       plain English -> structured policy (Claude API + schema validation + retry)
+- witness-services/      three independent witnesses, separate process / private key / data file
|  +- common/crypto.js    public encoding + signature spec, all parties agree
+- prover/                witness assembly, proof generation, off-chain verify, scenarios
+- contracts/             TradeExecutor (record + execute), MockAsset, Verifier.sol (auto-generated)
+- demo-ui/               live demo UI: streams the real pipeline execution
+- scripts/               key generation, on-chain deploy demo, one-shot demo
```
